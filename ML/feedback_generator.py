...
- DIP: Knee bend depth. Ideal 90-110 degrees.
- ASCENT: Torso posture. Ideal upright 165-180 degrees.
...
- DIP: Глубина сгиба коленей. Идеал 90-110 градусов.
- ASCENT: Осанка корпуса. Идеал прямо 165-180 градусов.
...
Metrics: knee={metrics.get('min_knee_dip','?')} deg, torso={metrics.get('torso_ascent','?')} deg, elbow={metrics.get('elbow_release','?')} deg, forearm={metrics.get('forearm_release','?')} deg, follow-through={metrics.get('frames_in_follow_through',0)} frames.
...
Метрики: колено={metrics.get('min_knee_dip','?')} deg, корпус={metrics.get('torso_ascent','?')} deg, локоть={metrics.get('elbow_release','?')} deg, предплечье={metrics.get('forearm_release','?')} deg, завершение={metrics.get('frames_in_follow_through',0)} кадров.
