...
IMPORTANT: Some metrics may have default/unreliable values if the pose tracker couldn't detect the body properly:
- knee angle = 180° means legs were straight (no squat detected) — this could mean the player didn't squat OR the tracker failed
- elbow angle = 0° means no release was detected — likely tracker issue, NOT a bad shot
- forearm = 90° means no forearm angle was captured
- follow-through frames = 0 means the shot cycle wasn't completed on camera
- elbow_snap = 0 means no follow-through data captured
- arm_stability_std = 0 means no follow-through data

When you see these default values, give the benefit of the doubt — the tracking may have been imperfect. Don't punish scores heavily for missing data. A score of 50 should be reserved for actually poor form, not missing data.

If MOST metrics are at default/unreliable values, give a neutral score around 65-75 and mention that tracking was limited.
...
ВАЖНО: Некоторые метрики могут иметь дефолтные/ненадёжные значения, если pose-трекер не смог определить тело:
- угол колена = 180° означает что ноги были прямыми (присед не обнаружен) — это может означать что игрок не присел ИЛИ трекер не сработал
- угол локтя = 0° означает что релиз не обнаружен — скорее проблема трекера, а НЕ плохой бросок
- предплечье = 90° означает что угол не был зафиксирован
- кадры завершения = 0 означает что цикл броска не был завершён на камере
- elbow_snap = 0 означает что данных о завершении нет
- arm_stability_std = 0 означает что данных о завершении нет

Когда видишь дефолтные значения — давайenefit of the doubt — трекинг мог быть не идеальным. Не наказывай баллы сильно за отсутствие данных. Оценка 50 должна быть за реально плохую технику, а не за отсутствие данных.

Если БОЛЬШИНСТВО метрик дефолтные/ненадёжные — дай нейтральную оценку 65-75 и упомяни что трекинг был ограничен.
...
def score_with_llm(metrics, lang="en"):
    ...
    # Detect if most metrics are unreliable (default values)
    unreliable_count = 0
    total_metrics = 5
    if metrics.get('min_knee_dip', 180) >= 175:
        unreliable_count += 1
    if metrics.get('elbow_release', 0) <= 5:
        unreliable_count += 1
    if metrics.get('forearm_release', 90) >= 85:
        unreliable_count += 1
    if metrics.get('frames_in_follow_through', 0) <= 1:
        unreliable_count += 1
    if metrics.get('elbow_snap', 0) <= 1:
        unreliable_count += 1

    tracking_quality = "GOOD"
    if unreliable_count >= 3:
        tracking_quality = "POOR - most metrics are at default values, tracking was limited"
    elif unreliable_count >= 2:
        tracking_quality = "PARTIAL - some metrics missing, score with caution"
...
Качество трекинга: {tracking_quality}
